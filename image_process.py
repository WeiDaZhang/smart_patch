...
class Image_process:
    ...
    def detect_probe_direction(self, img_capture, maxEllipse, points):
        ...
        if not len(edge_points) == 0: #if the edge_points list is NOT empty
            ...
            if not (np.isnan(x2) or np.isnan(y2)): 
                ...
                img_capture = cv.arrowedLine(img_capture,(x1,y1),(x2,y2),(255),thickness=10)
        ...
        return (x1,y1),(x2,y2), maxEllipse[2]
    ...
    def detect_probe_high_mag(self, img_capture):
        ...
        cv.drawContours(closing_img, hull_list, -1, (255), thickness=3)
        #cv.imshow('Convex Hull Fill',img_capture)
        #cv.waitKey()

        cv.fillPoly(closing_img, hull_list, (255))
        #cv.imshow('Convex Hull Fill Poly',img_capture)
        #cv.waitKey() 
        ...
        for i, c in enumerate(bounding_cnts):
            ...
            if c.shape[0] > 5:
                ...
                if not minEllipse[i] == None:
                    ...
                    screen_size = int(width)*int(height)
                    if area>tmp and area<(0.8*screen_size):   #(screen_size/2):
                        tmp=area
                        maxRect = minRect[i]
                        maxEllipse = minEllipse[i]
                        points[0][0] = box[0][0]
        ...
        return hull_thresh, maxRect, maxEllipse, points, tmp
