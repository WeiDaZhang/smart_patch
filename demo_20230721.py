...
def main():
    ...
    while True:
        ...
        if coordinate_calibration_flag:
            ...
            tip_sign = np.sign(centre_coord - tip_coord_most)
            if tip_sign[0] == 0:
                tip_sign[0] = 1
            if tip_sign[1] == 0:
                tip_sign[1] = 1
            ...
            pixel_2_patch_angle_new = np.arctan(tip_sign[1]/tip_sign[0]) - np.arctan(coord_cal_vector[1]/coord_cal_vector[0])
            #pixel_2_patch_angle_new = np.pi/2 - pixel_2_patch_angle_new
            print(f"scale = {pixel_2_patch_scale_new}")
            print(f"angle = {pixel_2_patch_angle_new}")

            # if update
            pixel_2_patch_scale = pixel_2_patch_scale_new
            pixel_2_patch_angle = pixel_2_patch_angle_new
