...
def split_index_list(input_list, max_len = 1000):
    item_cnt = len(input_list)
    for n_splt in range(1, 100):
        if not item_cnt / n_splt > max_len:
            n_residual = item_cnt % n_splt
            break
    if n_residual:
        idx_list = np.hstack((np.arange(item_cnt), np.ones(n_splt - n_residual) * (item_cnt - 1)))
    else:
        idx_list = np.arange(item_cnt)
    idx_list.shape = (np.ceil(item_cnt / n_splt).astype(int), n_splt)
    idx_list = np.floor(idx_list).astype(int)
    return idx_list
