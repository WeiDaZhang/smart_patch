...
def main():
    ...
    #patchstar_x,patchstar_y,patchstar_z = patchstar.moveRelative(patchstar_x,patchstar_y,patchstar_z,-500_00,0,0)
    ...
    #Movement loop
    slicescope_movement = 25_00
    slicescope_movement_list = np.array([[1,-1],[-2,0],[0,2],[2,0],[-1,-1]])*slicescope_movement
    slicescope_movement_idx = 0
    

    coordinate_calibration_flag = 1
    #Relative coordinate plane rotation/translation for patch to slicescope
    #Mechanical movement to pixel translation for patch
    pixel_2_patch_scale = 165.45
    pixel_2_patch_angle = np.deg2rad(16.3)  # angle in rad
    pixel_2_patch_scale = 163.8
    pixel_2_patch_angle = 0.25338  # angle in rad
    #Mechanical movement to pixel translation for slicescope
    pixel_2_scope_scale = 15.88
    pixel_2_scope_angle = 0
    ...
    while True:
        ...
        if coordinate_calibration_flag:
            coord_cal_displace = 120_00
            tip_sign = np.sign(tip_coord_most - centre_coord)
            if tip_sign[0] == 0:
                tip_sign[0] = 1
            if tip_sign[1] == 0:
                tip_sign[1] = 1
            print(f"tip sign: {tip_sign}")

            #img = cam.image_cross()
            #cv.imshow('Image',img)
            #cv.waitKey(0)

            #patchstar_x,patchstar_y,patchstar_z = patchstar.moveRelative(patchstar_x,patchstar_y,patchstar_z,int(coord_cal_displace*tip_sign[0]),int(coord_cal_displace*tip_sign[1]),0)
            slicescope_x,slicescope_y,slicescope_z = slicescope.moveRelative(slicescope_x,slicescope_y,slicescope_z,int(coord_cal_displace*tip_sign[0]),int(coord_cal_displace*tip_sign[1]),0)
            img = cam.image_cross()
            cv.imshow('Image',img)
            cv.waitKey(0)
            ...
            pixel_2_scope_scale_new = coord_cal_displace*np.sqrt(2)/np.linalg.norm(coord_cal_vector)
            pixel_2_scope_angle_new = np.arctan(tip_sign[1]/tip_sign[0]) - np.arctan(coord_cal_vector[1]/coord_cal_vector[0])
            #pixel_2_patch_angle_new = np.pi/2 - pixel_2_patch_angle_new
            print(f"scale = {pixel_2_scope_scale_new}")
            print(f"angle = {pixel_2_scope_angle_new}")

            # if update
            pixel_2_scope_scale = pixel_2_scope_scale_new
            pixel_2_scope_angle = pixel_2_scope_angle_new

            tip_coord_most = tip_2nd_coord_most
